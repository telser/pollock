...
decls :: ReportErrorMessage m => TcGblEnv -> m [(GenLocated SrcSpanAnnA (HsDecl GhcRn), [HsDoc GhcRn])]
decls tcGblEnv =
...
type Pollock_Maps = (DocMap Name, ArgMap Name,
...
pollock_mkExportItems
  is_sig pkgName thisMod semMod warnings gre exportedNames hsdecls
  maps unrestricted_imp_mods exportList allExports
  dflags =
  case exportList of
    Nothing      ->
      pollock_fullModuleContents is_sig pkgName thisMod semMod warnings gre
        exportedNames hsdecls maps dflags
        allExports
  ...
  where
    ...
    availExportDecl avail decl (doc, subs)
      | availExportsDecl avail = do
          ...
          return [ Pollock_ExportItemDecl $ Pollock_ExportDecl {
        ...
        case mtyThing of
          Just (AConLike PatSynCon{}) -> do
            ...
            pure [ (unLoc patsyn_decl, patsyn_doc)
                 | Pollock_ExportItemDecl (Pollock_ExportDecl {
...
pollock_hiValExportItem
  :: Monad m => DynFlags -> Name -> SrcSpan -> DocForDecl Name
  -> IfM m (Pollock_ExportItem GhcRn)
...
pollock_fullModuleContents
  ...
  -> Avails
  -> IfM m [Pollock_ExportItem GhcRn]
pollock_fullModuleContents is_sig pkgName thisMod semMod warnings gre exportedNames
  hsdecls maps@(_, _, declMap) dflags avails = do
  let availEnv = availsToNameEnv (nubAvails avails)
  (concat . concat) `fmap` (for hsdecls $ \decl -> do
    case decl of
      ...
      _ ->
        for (getMainDeclBinder emptyOccEnv (unLoc decl)) $ \nm -> do
          case lookupNameEnv availEnv nm of
            Just avail ->
              pollock_availExportItem is_sig thisMod
                semMod warnings exportedNames maps
                dflags avail
            Nothing -> pure [])
  where
